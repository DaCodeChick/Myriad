...
def register_dm_commands(bot: "MyriadDiscordBot") -> None:
    ...
    @bot.tree.command(
        name="dm",
        description="Narrate a scene using the narrator persona",
    )
    ...
    async def dm_narrate(interaction: discord.Interaction, narration: str):
        """
        Use the system narrator persona to deliver narration.

        The 'narrator' is a hardcoded system persona that exists to provide
        atmospheric third-person narration without a physical form.
        """
        ...
        try:
            ...
            # Temporarily switch to the narrator system persona
